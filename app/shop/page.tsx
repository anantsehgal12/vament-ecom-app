              ...
              <div key={product.id} className="group relative">
                <div className="aspect-h-1 aspect-w-1 w-full overflow-hidden rounded-md bg-gray-200 lg:aspect-none group-hover:opacity-75 h-48 md:h-64 lg:h-80">
                  {(() => {
                    ...
                    return image ? (
                      <img alt={image.alt || product.name} src={image.src} className="h-full w-full object-cover object-center" />
                    ) : null;
                  })()}
                </div>
                <div className="mt-4 flex justify-between">
                  ...
                  <p className="text-sm md:text-base font-medium text-white">
                    ₹
                    {Math.round(
                      parseFloat(product.price) + (parseFloat(product.price) * product.taxRate) / 100
                    ).toString()}
                  </p>
                </div>
              </div>
